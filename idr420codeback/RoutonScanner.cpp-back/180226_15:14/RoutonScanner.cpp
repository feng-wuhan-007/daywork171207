...
//fengmc add 180223
//iDR420 扫描模块上电
static int power_on_Barcode()
{
    ALOGI("%s()", __FUNCTION__);
    system("routon_client \"7e8802662e5daf979af0a3a5266bcaa1 @@CMD_BARCODE_ON\"");
    return 0;                                                                                                                                                                                               
}
//iDR420 扫描模块下电
static int power_off_Barcode()
{
    ALOGI("%s()", __FUNCTION__);
    system("routon_client \"7e8802662e5daf979af0a3a5266bcaa1 @@CMD_BARCODE_OFF\"");
    return 0;
}
static jstring iDR_CheckBarcodeType(JNIEnv *e, jobject o)
{
    ALOGI("%s()", __FUNCTION__);

    //stop BarcodeService
    system("routon_client \"7e8802662e5daf979af0a3a5266bcaa1 /system/bin/am broadcast -a  scanner_service_pause -n  com.geomobile.barcode/.BarcodeReceiver\"");
    
    //Power-on
    int ret = power_on_Barcode();
    int fd;
    unsigned char commandAsk[] = {0x3F};
    unsigned char recv[64];
    unsigned char buf[8];
    int len = 0;
    unsigned char readRegister[] = {0x7e,0x00,0x07,0x01,0x00,0x05,0x03,0xde,0xf6};
	...
	//open error  fd = -1
	    ...
	    ALOGE("open error. %s\n", strerror(errno)); 
            ...
            //检测二维				
            ret = write(fd,commandAsk,sizeof(commandAsk));
            if (ret > 0) {
                usleep(100000);//100ms
                //ALOGI("read = %d",read(fd,buf,sizeof(buf))); 
                while ((ret = read(fd,buf,sizeof(buf))) > 0) {
		    //20180226:增加容错,避免内存越界
                    if ((len + ret) < sizeof(recv)) {
                        memcpy(recv,buf,ret);						
                    }
                    else {
                        break;
                ...
                if (len > 0) {
                    close(fd);
                    power_off_Barcode();
                    ALOGI("code success 2 !!!");
                    //restart BarcodeService
                    system("routon_client \"7e8802662e5daf979af0a3a5266bcaa1 /system/bin/am broadcast -a  scanner_service_resume -n  com.geomobile.barcode/.BarcodeReceiver\"");
                    return e->NewStringUTF("2");
                }

                // 检测一维
                ret = write(fd,readRegister,sizeof(readRegister));
                if (ret > 0) {
                    usleep(100000);//100ms
                    while ((ret = read(fd,buf,sizeof(buf))) > 0){
                        //ALOGI("fengmc one barcode!!!");
                        memcpy(recv,buf,ret);                                 
                        len += ret;
                    }
                    ...
                    if (len > 0) {
                        close(fd);
                        power_off_Barcode();
                        ALOGI("code success 1 !!!");
                        //restart BarcodeService
                        system("routon_client \"7e8802662e5daf979af0a3a5266bcaa1 /system/bin/am broadcast -a  scanner_service_resume -n  com.geomobile.barcode/.BarcodeReceiver\"");
                        return e->NewStringUTF("1");
                    }
                 }
            }
...
static JNINativeMethod methods[] = {
    ...
    {"check_type", "()Ljava/lang/String;", (void *)iDR_CheckBarcodeType},//fengmc add 180211
