...
import android.media.SoundPool;//fengmc add 2017
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static private boolean scannersock=false;
    //static public Process myprocess;
    /**
...
//$_rbox_$_modify_$_end
    ...
    private void handleShortPressOnHome() {
        // hehj mark, disable homekey when choosing password
        if (!"0".equals(SystemProperties.get("routon.dishome"))) {
            Slog.w(TAG, "hehj disable HOME showing");
            return;
        }
        // end
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        filter.addAction("routon_lock_screen");
	...
	//Slog.d(TAG, "android.intent.action.brightnessbialog");
        Intent intent = new Intent();
        intent.setAction("android.intent.action.brightnessdialog");
    ...
    private static final int[] WINDOW_TYPES_WHERE_HOME_DOESNT_WORK = {
      ...
      //fengmc delete 180124
	/*
      SoundPool soundPool = new SoundPool(1,AudioManager.STREAM_MUSIC,0);
      int loadId=soundPool.load("/system/media/audio/ui/Trusted.ogg", 1);
	*/
      //end
    @Override
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
       ...
       //fengmc delete 180124
        /*if(event.getDevice().getName().equals("sc16is750")){
          
            soundPool.play(loadId, 2f, 2f, 1, 0, 1);
...
//                        i.putExtra("presstime",(now-mKeyDownTime));
	    	...
	    	int routon_scaner_enable = android.provider.Settings.Global.getInt(mContext.getContentResolver(), "routon_has_scaner", 2);
		//fengmc 180115
		//Log.d(TAG,"routon_scaner_enable="+routon_scaner_enable);
		//end
		//fengmc modify 180124
		//if(routon_scaner_enable == 1 || routon_scaner_enable == 2){
		if(routon_scaner_enable == 1 || routon_scaner_enable == 2||routon_scaner_enable==0){
		//end 
			WindowManager.LayoutParams attrs_scanner = win != null ? win.getAttrs() : null;
			if( (attrs_scanner != null) && !attrs_scanner.packageName.equals("com.android.launcher3")){
	                	if(down && !scannersock) {
					scannersock=true;
					try{
						//fengmc add F10_key_start 180124
						Runtime.getRuntime().exec("/system/bin/scanner_begin.sh F10_key_start");
						//end
					}catch(IOException t){
					...
					try{
						 //fengmc add F10_key_end  180124
						Runtime.getRuntime().exec("/system/bin/scanner_end.sh F10_key_end");
						//end
					}catch(IOException t){
						t.printStackTrace();
					}
...
//                        i.putExtra("presstime",(now-mKeyDownTime));
	...
	/*********just add for idr420**************/
/*	
	    case KeyEvent.KEYCODE_F1ce()0:
	    	int routon_scaner_enable = android.provider.Settings.Global.getInt(mContext.getContentResolver(),"routon_has_scaner",0);
		if(routon_scaner_enable==1){
			WindowManager.LayoutParams attrs_scanner = wins != null ? wins.getAttrs() : null;
			if( (attrs_scanner != null) && !attrs_scanner.packageName.equals("com.android.Launcher3")){
	                	if(down) {
					try{
						Runtime.getRuntime().exec("/system/bin/scanner_begin.sh");
					}catch(IOException t){
						t.printStackTrace();
					}
				}
					...
					try{
						Runtime.getRuntime().exec("/system/bin/scanner_end.sh");
					}catch(IOException t){
						t.printStackTrace();
					}
				}
...
*/
    ...
    BroadcastReceiver mMultiuserReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if (intent.getAction().equals("routon_lock_screen")) {
                // hehj GA lock screen 
                lockNow(null);
            }
        }
